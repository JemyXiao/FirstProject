    ...
    public static void validate(Field field,Object object) throws Exception{
        ...
        if(!dv.nullable()){
            if(value==null|| StringUtils.isEmpty(value)){
                throw new RecruitValidationException(description+"不能为空");
            }
        }

        if(value.toString().length()>dv.maxLength()&&dv.maxLength()!=0){
            throw new RecruitValidationException(description+"长度不能超过"+dv.maxLength());
        }

        if(value.toString().length()<dv.minLength()&&dv.minLength()!=0){
            throw new RecruitValidationException(description+"长度不能小于"+dv.minLength());
        }
            ...
            switch (dv.regexType()) {
                ...
                case SPECIALCHAR:
                    if(RegexUtils.hasSpecialChar(value.toString())){
                        throw new RecruitValidationException(description+"不能含有特殊字符");
                    }
                    break;
                case CHINESE:
                    if(RegexUtils.isChinese2(value.toString())){
                        throw new RecruitValidationException(description+"不能含有中文字符");
                    }
                    break;
                case EMAIL:
                    if(!RegexUtils.isEmail(value.toString())){
                        throw new RecruitValidationException(description+"地址格式不正确");
                    }
                    break;
                case IP:
                ...
                case NUMBER:
                    if(!RegexUtils.isNumber(value.toString())){
                        throw new RecruitValidationException(description+"不是数字");
                    }
                    break;
                case PHONENUMBER:
        ...
        if(!dv.regexExpression().equals("")){
            if(value.toString().matches(dv.regexExpression())){
                throw new RecruitValidationException(description+"格式不正确");
            }
        }
        /*************注解解析工作结束******************/
    }
